...
save_Fig = False
show_Fig = True
filename_panela = 'figure5a_re500.png'
filename_panelb = 'figure5b_re500.png'
# USER INPUT PARAMETERS
cmap = cm._colormaps['Spectral_r']
zlocs_target = [20,40,80,150,300,450]
...
H = 1.0
utau = 5.0e-4
zloc_time_spectrum = 198
# CANSIC
Euu_cansic = np.load('data/Re500/spectrum_cansic.npy')
freqx_cansic = np.load('data/Re500/freqx_cansic.npy')
# LINIC
Euu_linic = np.load('data/Re500/spectrum_linic.npy')
freqx_linic = np.load('data/Re500/freqx_linic.npy')
# SYNIC
Euu_synic = np.load('data/Re500/spectrum_synthetic.npy')
freqx_synic = np.load('data/Re500/freqx_synthetic.npy')
endInd = 239
data = np.loadtxt('data/Re500/grid.out')
z = data[1:-1,1]*Retau
zend = np.where(freqx_cansic<600)[0][-1]
# Find the locations of the zlocs
zlocs = np.zeros((len(zlocs_target),1))
for i in range(0,len(zlocs_target)):
    zlocs[i] = np.where(z<=zlocs_target[i])[0][-1]
    print(zlocs[i,0]," -- Location")
print("Location of plotting: ",z[zloc])
print("Size of datasets")
print("CansIC:",np.shape(Euu_cansic))
print("LinearIC:",np.shape(Euu_linic))
...
for iter in range(0, len(zlocs)):
    ...
    # -5/3rd
    #ax.loglog(freqx_linic[4:60], 8e-2 * (freqx_linic[4:60])**(-5 / 3.0), 'r--')
